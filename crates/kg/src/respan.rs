...
pub fn respan_generation(
  ...
  // reach.bin links (a respan admits only span-shifting edits, so the include-edge
  // set is unchanged — and the scratch twin always writes it, so id convergence
  // requires it present) ----
  for cache in ["names.idx", "graph.bin", "reach.bin"] {
