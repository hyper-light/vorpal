...
pub fn compose_defs_changed(
  ...
  // reach.bin: import-stable across every admitted compose (the scoped session
  // declines when a file's imports diverge) — hard-link the prior's bytes; the
  // Merkle fold requires the artifact present for id convergence.
  {
