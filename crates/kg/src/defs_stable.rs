...
pub fn compose_defs_stable(
  ...
  // reach.bin: the include-edge graph is import-stable across every admitted
  // compose (the scoped session declines when a file's imports diverge from
  // it), so the carried bytes are the scratch twin's bytes — and the Merkle
  // fold requires it present for id convergence.
  let (from, to) = (prior.join("reach.bin"), staging.join("reach.bin"));
  if from.exists() {
    let _ = fs::remove_file(&to);
    if fs::hard_link(&from, &to).is_err() {
      fs::copy(&from, &to)?;
