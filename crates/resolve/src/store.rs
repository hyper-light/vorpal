...
/// One file's footprint in the store: its contiguous record range and its import
/// references, retained encoded (interner-free) for the link phase's two pre-passes
/// (import-binding seed; include-reach edges).
struct FileRefs {
  records: Range<u64>,
  imports: Vec<u8>,
}
...
impl RefStore {
  ...
  /// The alive import references (every form — symbol imports seed bindings, path
  /// imports build the include-reach graph; the same retention law as the spill's
  /// `imports()`), decoded against `interner`, **in the caller's
  ...
  pub fn imports<'i>(
    ...
    for path_bits in order {
      ...
      for record in file.imports.chunks_exact(RECORD) {
  ...
  /// [`RefStore::imports`] for why order is the caller's). Chunks never span two
...
mod tests {
  ...
  fn imports_track_retirement() {
    ...
    let qualified = Reference::new(itn(), NodeId::new(2), "m.py", "pkg", RefKind::Import)
      .with_form(crate::reference::RefForm::Static)
