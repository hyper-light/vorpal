...
fn sg_label_to_code_span_label(label: vorpal_config::Label<impl Doc>) -> Label<()> {
  let ret = match label.style {
    vorpal_config::LabelStyle::Primary => Label::primary((), label.range()),
    vorpal_config::LabelStyle::Secondary => Label::secondary((), label.range()),
  };
