...
injected: [js, ts, tsx]";
  ...
  fn test_good_injection() {
    ...
    let sg = lang.grep("const a = styled`.btn { margin: 0; }`");
    let root = sg.root();
    extract_custom_inject(&lang, &injections, root, &mut ret);
    assert_eq!(ret.len(), 1);
    ...
    ret.clear();
    let sg = lang.grep("const a = styled.css`.btn { margin: 0; }`");
    let root = sg.root();
    extract_custom_inject(&lang, &injections, root, &mut ret);
    assert_eq!(ret.len(), 1);
    assert_eq!(ret[0].0, "css");
  ...
  fn test_multiple_matches_produce_separate_entries() {
    ...
    let lang = SgLang::from(SupportLang::JavaScript);
    let sg =
      lang.grep("const a = styled`.btn { margin: 0; }`; const b = styled`.card { padding: 1em; }`");
    let root = sg.root();
    extract_custom_inject(&lang, &injections, root, &mut ret);
    assert_eq!(ret.len(), 2);
    assert_eq!(ret[0].0, "css");
