...
pub fn release_freed_pages() {
  ...
  // (Restored after the two-mains merge silently reverted this body to the zone call —
  // resident peak had crept 4.2 → 6.7 GB with nobody noticing, because the fault counters
  // decay-off protects stayed low. The purge now runs on a DETACHED thread: madvise is
  // thread-safe against concurrent allocation, so the seal never waits on page release.)
  // The macOS `malloc_zone_pressure_relief` addresses the SYSTEM allocator, which a
...
#[allow(clippy::too_many_arguments)] // the one spilled stream entry: spill paths + pack + fresh sink are all load-bearing
...
#[allow(clippy::too_many_arguments)] // the shared stream body behind the three public entries
