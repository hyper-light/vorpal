...
pub type LinkedRetained = std::io::Result<(
  ...
  // The encoded include-edge graph (`reach.bin` bytes) — the persist writes it so
  // scoped composes over a daemon-committed generation replay the same oracle a
  // scratch build persists.
  Vec<u8>,
)>;
...
impl RetainedIndex {
  ...
  pub fn link_for_serving(
    ...
    let (kg, stats, _, _, _, _) = self.link_inner(interner, resolver, false, pre_edges)?;
    Ok((kg, stats))
  }
  ...
  fn link_inner(
    ...
    let table = self
      ...
      .borrow_mut(interner);
    // The spilled pipeline's pre-passes, verbatim (predicate parity is load-bearing —
    // the persisted generation must equal a scratch build's): every alive import feeds
    // root learning, the binding seed, and the include-reach edges.
    let imports = self.store.imports(interner, order.iter().copied());
    table.learn_include_roots(interner, &imports);
    vorpal_resolve::seed_import_bindings(interner, table, &imports, resolver);
    let include_edges = vorpal_resolve::include_edges(interner, table, &imports);
    ...
    let ok = untouched
      ...
      .map(|bucket| {
        ...
        for row in &mut bucket.evidence {
          ...
          for alt in row.alternatives.iter_mut() {
  ...
  fn assemble(
    ...
    if !want_evidence {
      return Ok((kg, stats, Vec::new(), flows, sig_family_rows, reach_graph));
    }
    ...
    Ok((kg, stats, evidence, flows, sig_family_rows, reach_graph))
  }
}
