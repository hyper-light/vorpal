...
fn retained_edits_link_identical_to_scratch_pipeline() {
  ...
  let (kg_live, stats_live, evidence_live, _flows_live, _sigs_live, _reach_live) = retained
  ...
  let (_kg_fresh, _stats_fresh, evidence_fresh, _flows_fresh, _sigs_fresh, _reach_fresh) = fresh
    .link(itn(), &Resolver::new(), &[])
    .expect("fresh link");
