...
fn defs_changed_session_equals_scratch_for_the_closure() {
  ...
  // The compose's posture exactly: the session replays the prior generation's
  // persisted include-edge graph (reach=None is a hard decline, tested below nowhere —
  // production falls back to the full pipeline on that path).
  let reach_bytes =
    fs::read(gen_prior.join(vorpal_ingest::REACH_GRAPH_FILE)).expect("prior reach.bin");
  let reach = vorpal_ingest::ReachGraph::decode(&reach_bytes).expect("reach graph");
