...
// PEP 810: identifier-continuation test (ASCII). Used only to enforce a word boundary
// around `lazy`/`import`/`from`; any non-ASCII continuation simply falls through to a
// negative result, which is correct — `lazy<letter>` is a different identifier and can
// never be a lazy import.
...
bool tree_sitter_python_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    ...
    // PEP 810 lazy imports. Reached only after all layout tokens (NEWLINE/INDENT/DEDENT) are
    // resolved, with the lexer positioned on the first content character. `lazy` becomes the
    // keyword ONLY when the next significant word is the reserved `import`/`from`; otherwise
    // we return false and the internal lexer tokenizes `lazy` as an ordinary identifier — so
    // `lazy = 1`, `def lazy()`, `x.lazy`, `import x as lazy`, etc. are all unaffected. The
    // grammar alone cannot express this: a soft keyword before a reserved word produces no GLR
    // fork, so the lexer would otherwise pre-commit `lazy` and break its use as an identifier.
    if (valid_symbols[LAZY] && lexer->lookahead == 'l') {
        static const char kw_lazy[] = "lazy";
        bool matched = true;
        for (int i = 0; i < 4; i++) {
            if (lexer->lookahead != kw_lazy[i]) {
                matched = false;
                break;
            }
            advance(lexer);
        }
        // `lazy` must be a whole word (not the prefix of `lazyload`, `lazily`, ...).
        if (matched && !is_ident_continue(lexer->lookahead)) {
            lexer->mark_end(lexer); // the emitted token spans exactly "lazy"
            // Skip inline whitespace and explicit line continuations to reach the word that
            // decides whether this `lazy` is a modifier. A bare newline ends the statement,
            // so it (correctly) fails the check below and `lazy` stays an identifier.
            for (;;) {
            ...
            if (follow) {
                bool follows = true;
                for (int i = 0; follow[i]; i++) {
                    if (lexer->lookahead != follow[i]) {
                        follows = false;
                        break;
                    }
                    advance(lexer);
                }
                if (follows && !is_ident_continue(lexer->lookahead)) {
                    lexer->result_symbol = LAZY;
                    return true; // token already bounded to "lazy" by mark_end above
                }
            }
        }
        // Starts with `l` where LAZY is the only producible external token; letting the
        // internal lexer read the identifier is always correct here.
        return false;
    }
