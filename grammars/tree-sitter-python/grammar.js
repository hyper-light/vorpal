...
module.exports = grammar({
  ...
  externals: $ => [
    ...
    // PEP 810 lazy imports. `lazy` is a *soft* keyword followed by the *reserved*
    // words `import`/`from` — the first such case in Python, which the grammar alone
    // cannot express (no GLR fork is generated, so the lexer would pre-commit `lazy`
    // and break its use as an identifier). The scanner emits this token only when the
    // next significant word is `import`/`from`; otherwise `lazy` lexes as an identifier.
    $.lazy,
  ],
  ...
  rules: {
    ...
    // PEP 810 (CPython 3.15): an optional `lazy` soft-keyword prefix. `lazy` is emitted
    // by the external scanner ONLY when immediately followed by `import`/`from`; everywhere
    // else it stays an ordinary identifier. See src/scanner.c and the `externals` list.
