...
/* vorpal local patch (recorded in grammars/PROVENANCE.json): the upstream embedded copy of
 * tree-sitter-html's scanner also EXPORTED the five `tree_sitter_html_external_scanner_*`
 * C symbols. Linked next to the real tree-sitter-html static library, those duplicate
 * definitions let the linker resolve HTML's scanner to this 2020-era copy — silently breaking
 * HTML parsing corpus-wide. Vue only needs the `Scanner` class from this include; the extern
 * block is deleted. */
