...
arena_i_decay_ms_ctl_impl(tsd_t *tsd, const size_t *mib, size_t miblen,
	...
	/*
	 * MALLCTL_ARENAS_ALL: apply a write to every initialized arena,
	 * mirroring arena_i_decay().  The mib resolver (arena_i_index) admits
	 * the sentinel into this handler, but the unchecked arena_get() below
	 * indexed one past the arenas array (MALLOCX_ARENA_LIMIT slots) and
	 * dereferenced garbage.  A read of ALL has no single answer: EINVAL.
	 */
	if (arena_ind == MALLCTL_ARENAS_ALL) {
		if (oldp != NULL && oldlenp != NULL) {
			ret = EINVAL;
		...
		if (newp != NULL) {
			...
			malloc_mutex_unlock(tsd_tsdn(tsd), &ctl_mtx);
			for (unsigned i = 0; i < narenas; i++) {
				arena = arena_get(tsd_tsdn(tsd), i, false);
				if (arena != NULL && arena_decay_ms_set(
				        tsd_tsdn(tsd), arena, state,
				        *(ssize_t *)newp)) {
					ret = EFAULT;
					goto label_return;
				}
