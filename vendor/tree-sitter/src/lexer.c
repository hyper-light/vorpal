...
static void ts_lexer__get_lookahead(Lexer *self) {
  ...
  // ASCII fast path (vorpal): source code is overwhelmingly ASCII, and for a UTF-8 input a
  // byte < 0x80 is a single self-decoding codepoint. Handling it inline here skips the
  // encoding dispatch and — more importantly — the indirect `decode(...)` call that otherwise
  // runs for every character the lexer consumes. Byte-identical to `ts_decode_utf8`, which
  // returns (lookahead = byte, size = 1) for the same input.
